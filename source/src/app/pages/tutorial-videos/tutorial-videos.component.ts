...
canEdit = false;
  ...
  getTableData$(pageNumber: Number, pageSize: Number, filter: string) {
    return this.commonService.getDatatableFilter('tutorial-videos/getDatatable', pageNumber, pageSize, filter);
  }
 ngOnInit() {
    ...
    Promise.resolve().then(() => {
      this.canAdd =
        this.permissionService
          .hasPermission('Tutorial Videos_Add');
      this.canEdit =
        this.permissionService
          .hasPermission('Tutorial Videos_Edit');

      this.canDelete =
        this.permissionService
          .hasPermission('Tutorial Videos_Delete');
  ...
  deleteCompany(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.commonService.postData('tutorial-videos/delete/' + id,{}).subscribe(
          response => {
