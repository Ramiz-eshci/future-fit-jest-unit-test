...
@Component({
  ...
  templateUrl: './be14-form.component.html',
  styleUrl: './be14-form.component.scss'
})
export class Be14FormComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    console.log('Received fitEntryId from BE13:', this.fitEntryId);
    ...
    this.employee.controls.forEach((employeeGroup: AbstractControl, employeeIndex: number) => {
  ...
  calculateSiteFitness(employeeIndex: number, inputIndex: number): void {
    ...
    const h = inputGroup.get('design_involvement')?.value;
    const i = inputGroup.get('issue_scope_inclusive')?.value;
    const j = inputGroup.get('timely_resolution')?.value;
    const X = (h === true && i === true && j === true) ? 1 : 0;

    let Y = 0;
    const criteriaPairs = [
      ['active_communication', 'confidentiality_protection'],
      ['responsibility_assigned'],
      ['independent_advice_access'],
      ['full_information_during_process'],
      ['consulted_on_changes'],
      ['feedback_requested', 'performance_monitored', 'feedback_included_in_assessment', 'improvements_implemented']
    ];

    const firstPair = criteriaPairs[0];
    if (firstPair.every(field => inputGroup.get(field)?.value === true)) Y += 1;

    for (let i = 1; i < criteriaPairs.length; i++) {
      const group = criteriaPairs[i];
      const matchCount = group.filter(field => inputGroup.get(field)?.value === true).length;
      if (matchCount === group.length) Y += 1;
    }

    const scoringTable = [0.3, 0.45, 0.6, 0.7, 0.8, 0.9, 1];
    const Z = scoringTable[Y] ?? 0;
    const AA = X * Z;
    const percentage = Math.round(AA * 100) + '%';

    inputGroup.get('employee_fitness_percentage')?.setValue(percentage, { emitEvent: false });
    this.calculateBE14DataCompleteness();
    this.formatCalculatedEmployeeFields(inputGroup);
  }

  calculateBE14DataCompleteness(): string {
  ...
  calculateProgressIndicator(year: number): string {
    ...
    this.employee.controls.forEach(siteGroup => {
      ...
      fitnessInputs.controls.forEach(inputGroup => {
        ...
        const relevance = inputGroup.get('relevance')?.value;
        let fitness = inputGroup.get('employee_fitness_percentage')?.value;
        ...
        if (relevance == 1 && employeeCount > 0 && fitness !== null && fitness !== '' && fitness !== undefined) {
  ...
  submitForm(showMessageAndRedirect: boolean = true) {
    ...
    console.log('BE14 Form Submitted:', formData);

    this.commonService.addData('be-form/submit/be14', formData).subscribe(
      (response: any) => {
  ...
  addFitnessInput(i: number): void {
    ...
    const newFitnessGroup = this.createFitnessInputBe14();
